...
#shuriken spritesheet
shuriken_sheet_img = pygame.image.load('assets/shuriken.png').convert_alpha()
shuriken_sheet = SpriteSheet(shuriken_sheet_img)
...
#player instance
jump = Player(SCREEN_WIDTH // 2, SCREEN_HEIGHT - 150)
...
while run:
	...
	if game_over == False:
		...
		if len(enemy_group) == 0 and score > 1500:
			enemy = Enemy(SCREEN_WIDTH, 100, shuriken_sheet, 1.5)
		...
		#check game over
		if jump.rect.top > SCREEN_HEIGHT:
			game_over = True
			death_fx.play()
		#check for collision with enemies
		if pygame.sprite.spritecollide(jump, enemy_group, False):
			if pygame.sprite.spritecollide(jump, enemy_group, False, pygame.sprite.collide_mask):
				game_over = True
				death_fx.play()
	else:
		if fade_counter < SCREEN_WIDTH:
			...
			if key[pygame.K_SPACE]:
				...
				fade_counter = 0
				#reposition jump
				jump.rect.center = (SCREEN_WIDTH // 2, SCREEN_HEIGHT - 150)
				#reset enemies
				enemy_group.empty()
				#reset platforms
